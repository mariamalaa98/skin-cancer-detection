...
class MelBalancedImgDataset(IterableDataset):
    LABEL_IDX = 1
    IMG_PATH_IDX = 0
...
class MelHam1000Dataset(Dataset):
    LABEL_IDX = 1
    IMG_PATH_IDX = 0

    def __init__(self, csv_path, dataset_path, transform=None):
        self.full_data = self.get_dataframe(csv_path)

        self.transform = transform
        self.dataset_path = dataset_path
        self.no_rows = len(self.full_data)
    ...
    def __getitem__(self, i):
        path = f"{self.dataset_path}/{self.full_data[i][self.IMG_PATH_IDX]}.jpg"
        img = Image.open(path)
        if self.transform is not None:
            img = self.transform(img)
        patient_data = self.full_data[i][2:].astype(float)

        return img, torch.tensor(patient_data, dtype=torch.float), torch.tensor([self.full_data[i][self.LABEL_IDX]],
                                                                                dtype=torch.float)
...
class MelMergedDataset(MelBalancedImgDataset):
